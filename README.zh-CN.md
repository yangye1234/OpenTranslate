...
- 截图翻译：
  ...
  - Windows：有 C++/WinRT 时使用原生 Windows OCR，否则使用 PowerShell Windows Runtime OCR fallback
- 翻译缓存支持启用、清理、导入、导出。
...
- Windows 截图 OCR 优先使用带 Windows SDK C++/WinRT 头文件的 MSVC Qt Kit。如果找不到 `winrt/Windows.Foundation.h`，OpenTranslate 会改用 PowerShell Windows Runtime OCR fallback；安装 Windows SDK C++/WinRT 头文件后重新配置 CMake 可启用原生路径。
